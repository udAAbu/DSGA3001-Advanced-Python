...
std::vector<std::vector<double> > matmul(std::vector< std::vector<double > > &x, std::vector< std::vector<double > > &y){
	int m1 = x.size();
	...
	std::vector < std::vector < double > > z(m1, std::vector< double >(n2,0));
...
double calculate_sum(std::vector< std::vector<double> > &z){
	int n = z.size();
	double ans = 0;
